...
class Test58799(unittest.TestCase):
    def setUp(self):
        ver='v1_141'; mjd='58799';  axis="rot"
#        args='-%s -dir data/58799 -fiducialFile=data/v1_141/%s.dat -mjd 58799 ' % (axis, axis)
        path=os.path.dirname(sys.argv[0])
        args='-rot -dir %s/data/58799 -mjd 58799 ' % path
        args=args+'-time0 1573542725 -time1 1573545137 '
        args=args+' -canon -setCanon 80 -reset -scale ' 
        args=args+' -noplot'  
        self.fiducials, self.fpos, self.fposErr, self.nfpos = plotMcpFiducials.main(args.split())
        self.expect = read_fiducials('%s/data/v1_141/rot.dat'% path) 
    ...
    def test_encoder2(self,):
        ...
        #np.testing.assert_allclose(self.fpos['pos2'][1:], self.expect['Encoder2'], atol=0.1, err_msg="fpos['pos2'] failed")
