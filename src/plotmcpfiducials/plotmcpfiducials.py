...
import gzip
from sdsstools.time import get_sjd   #from opscore.utility import astrotime
...
def reset_fiducials(fididx, ud_delta, velocity, pos, verbose, axisName, resetFid):
    ...
    #        ff1=ff+ind 
    #        tmp = np.arange(len(pos))[np.logical_or(fididx == ff1, fididx == ff1 + ud_delta)]
    ...
    #        print "Optimized indices of reset fiducials, ff=%d,  %s " %  (ff,tmp), " (%s)" % len(tmp)
...
#  have this fiducial.  For each sweep we have been check if reset fid exist, if yes, ok. 
...
# the result looks not good. 
    ...
    for s in range(nsweep):
        ...
        for s0 in range(s-1, -2, -1):   # EM range(start, stop, step)
            #print "    testing low sweep (s0) =", s0, " has correction? ", corrections.has_key(s0) 
...
def get_input_file(filename, mjd, mjd_dir_fmt, verbose=False):
...
def set_canonical_fiducials(axisName, vecs, setCanonical):
    ...
    if setCanonical:
        ...
        if axisName == 'azimuth':   ff = 19
        elif axisName == 'altitude':  ff = 1; 
        elif axisName == 'rotator':   ff = 80; 
        ffval=(vecs["true1"][vecs["fididx"] == ff][0])              
        ss="(default)" 
    ...
    for n in ('pos1','pos2'):
        ...
        for i in range(len(fiducials)):
            ...
            if fiducials_deg[i] < -999 or not np.isfinite(fiducials_deg[i]):
                continue            
            #tmp = fiducials[np.abs(fiducials_deg - 360 - fiducials_deg[i]) < 1]  # warning
                 # RuntimeWarning: invalid value encountered in less
            ...
            match[i] = tmp[0]
            #print "   --", fiducials[i], fiducials_deg[i],  match[i],  fiducials_deg[match[i]] # , tmp
            ...
            #if verbose:
            #    print  fiducials[i], fiducials_deg[i], "--> ", match[i], fiducials_deg[match[i]]
...
def write_table_file(tableFile, fiducials, fpos, fposErr, nfpos,
    ...
    if args.mjd: ss=ss+" -mjd %i" % args.mjd
    ss=ss+" -%s" % axisName
    if args.time0: 
    ...
    if args.reset: ss=ss+" -reset"
    if args.scale: ss=ss+" -scale" 
    ...
    print("#",file=fd)
    print("# Fiducial Encoder1 +-   error  npoint  Encoder2 +-   error  npoint",file=fd)
        
    for i in range(1,len(fpos['pos1'])):
        print("%-4d" % fiducials[i], \
        "   %10.0f +- %7.1f %3d" % (fpos['pos1'][i], fposErr['pos1'][i], nfpos['pos1'][i]), \
        "   %10.0f +- %7.1f %3d" % (fpos['pos2'][i], fposErr['pos2'][i], nfpos['pos2'][i]), \
        "", file=fd)
...
def main(argv=None):
    ...
    parser.add_argument("dateFile", type=str, nargs="?", help="The mcpFiducials file to read")
    ...
    parser.add_argument("-dir", default="", dest="mjd_dir_fmt",
...
""")
    ...
    parser.add_argument("-resetFid",  type=int, help="reset fiducial index if default oe is not good")
    ...
    if args.verbose:
        ...
        print ("Program: ", "%s/%s" % (os.path.dirname(os.path.realpath(__file__)), os.path.basename(__file__)))
    ...
    dfile = get_input_file(args.dateFile, args.mjd, args.mjd_dir_fmt, verbose=args.verbose)
    if args.verbose:
    ...
    #  E<: JP has "True", and we had fiducials starting from 0. Changed to False. Code from proc:
    #fiducials = make_fiducials_vector(vecs['fididx'], False) #  EM: np.array(sorted(set(fididx)),dtype=int)
    fiducials = make_fiducials_vector(vecs['fididx'], True) #  JP : np.arange(0, max(fididx)+1)  
    ...
    if args.verbose:
        ...
        for i in range(1, len(fiducials)):
            #if np.isfinite(fiducials_deg[i]):  # why JP used this? 
                tmp2= vecs['pos1'][vecs['fididx'] == fiducials[i]]
                if len(tmp2) ==0:
                    ss= np.nan    
                else: 
                    ss= np.mean(tmp2)   
                print (" %3d\t%8.3f  %2d  %10.0f " % (fiducials[i], fiducials_deg[i], len(tmp2), ss))
    ...
    if args.error:
        ...
        if args.reset:
            reset_fiducials(vecs['fididx'], ud_delta, vecs['velocity'], vecs['pos1'], args.verbose,axisName,args.resetFid)
            reset_fiducials(vecs['fididx'], ud_delta, vecs['velocity'], vecs['pos2'], False,axisName, args.resetFid) # don't double-print
            ...
            for i in range(len(fiducials)):
                for pos in ('pos1','pos2'):
                    ...
                    if len(tmp) == 0:
                        ...
                        nfpos[pos][i] = len(tmp)
                             # changed statistics np.std(tmp) -->  np.std(tmp)/math.sqrt(nfpos[pos][i]-1)
                        fposErr[pos][i] = 1e10 if len(tmp) == 1 else np.std(tmp)/math.sqrt(nfpos[pos][i]-1)
    ...
    if not args.absolute:
        if args.verbose:
            print ("args.absolute=%s, change vecs[pos] to relative values" %  args.absolute)
        for pos in ('pos1','pos2'):
    ...
    if args.canonical:
        ...
        for pos in ('pos1','pos2'):
            ...
            #tmp = tmp[tmp != 0]     # valid crossings aren't == 0?   - reset fid has zero encod value
            if len(tmp) == 0:
    ...
    if args.noplot:
        ...
        if pyplot is None:
            ...
            sys.exit(1)
        plot_data(axisName, vecs, index, args.plotFile, dfile, ms, args.reset, args.updown, args.error, time0, tbase)
